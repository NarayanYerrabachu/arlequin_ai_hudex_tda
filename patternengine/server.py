"""TDA engine FastAPI server — same REST contract as the baseline workspace."""
...
def _build_state(docs):
    eng = TDAPatternEngine().ingest(docs)
    result = eng.run()
    ...
    meta = {
        ...
        "ripser_available": result["ripser_available"],
        "giotto_available": result["giotto_available"],
        "neo4j_available": result["neo4j_available"],
        "theme_quality": 0.0,
        "n_themes": result["n_clusters"],
        "tda": result["tda"],
    }
...
app = FastAPI(title="HuDex TDA Pattern Intelligence")


@app.get("/", response_class=HTMLResponse)
def index():
    html = (_HERE.parent / "hudex_tda.html").read_text(encoding="utf-8")
